...
const UpdateUser = () => {
    ...
    const [register, { data: registerData, loading, error }] = useMutation(UPDATE_USER)
    ...
    useEffect(() => {
        ...
        if (!registerData && error) {
            setModal({
                message: `Chỉnh sửa tài khoản thất bại, refresh lại trang để thử lại !`,
                active: true
            })
        }
    }, [registerData, error])
    ...
    return (
        <SubCard title="Chỉnh sửa tài khoản">
                ...
                <Grid container spacing={2}>
                    <Grid item lg={7}>
                        <Grid container spacing={2}>
                            ...
                            <Grid xs={12} xl={6} item>
                                <TextField
                                    ...
                                    label="Phân quyền"
                                    fullWidth
                                    value={formData.roles ? formData.roles[1] ?? formData.roles[0] : ''}
                        ...
                        <Box sx={{ m: "24px 4px", position: 'relative' }}>
                            <Button variant="contained" type="submit" fullWidth disabled={loading}>Chỉnh sửa tài khoản</Button>
                            {loading && (
                                <CircularProgress
                                    size={24}
                                    sx={{
                    ...
                    <Grid item lg={5}>
                        ...
                        <Box>
                            <Typography marginBottom={1}>
                                - Số điện thoại dùng làm mã định danh, không thể sửa số điện thoại
                            </Typography>
                        </Box>
                        <Box>
                            <Typography marginBottom={1}>
                                - Các trường không chỉnh sửa sẽ vẫn giữ nguyên giá trị
                            </Typography>
                        </Box>
                    </Grid>
                </Grid>
